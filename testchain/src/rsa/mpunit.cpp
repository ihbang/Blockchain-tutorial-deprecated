...
mpuint::mpuint(const mpuint &n) {
  ...
  unsigned i;
  for (i = 0; i < length; i++) value[i] = n.value[i];
}
...
void mpuint::operator=(const mpuint &n) {
  unsigned i;
  for (i = 0; i < length && i < n.length; i++) value[i] = n.value[i];
  for (; i < length; i++) value[i] = 0;
  for (; i < n.length; i++) {
    if (n.value[i] != 0) numeric_overflow();
  }
}
...
int mpuint::Compare(const mpuint &n) const {
  ...
  if (length > n.length) {
    ...
    for (i = n.length - 1; i >= length; i--) {
      if (n.value[i] != 0) return -1;
    }
  } else
    i = length - 1;
  while (true) {
    if (value[i] > n.value[i]) return 1;
    if (value[i] < n.value[i]) return -1;
    if (i == 0) return 0;
    i--;
  }
...
void mpuint::shift(unsigned bit) {
  ...
  if (bit != 0) numeric_overflow();
}

void mpuint::Divide(const mpuint &dividend, const mpuint &divisor,
                    mpuint &quotient, mpuint &remainder) {
  if (divisor.IsZero()) numeric_overflow();
  remainder = 0;
  quotient = 0;
...
void mpuint::dump() const {
  unsigned i;
  for (i = 0; i < length; i++) printf(" %x", value[i]);
  putchar('\n');
}