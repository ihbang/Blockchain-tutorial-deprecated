...
RSA::RSA(int256_t p, int256_t q, int256_t r)
    : p(0),
      q(0),
      m(0),
      phi_of_m(0),
...
const PublicKey* RSA::getPublicKey() const { return public_key; }

const PrivateKey* RSA::getPrivateKey() const { return private_key; }

bool RSA::isPrime(int256_t numb) {
  int it;
  for (it = 2; it < numb; it++) {
    if ((numb % it) == 0) return false;
  }
